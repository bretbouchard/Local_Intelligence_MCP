...
class HealthCheckTool: BaseMCPTool, @unchecked Sendable {
    ...
    init(logger: Logger, securityManager: SecurityManager) {
        let inputSchema: [String: Any] = [
            ...
            "description": "Perform comprehensive health check of the Local Intelligence MCP server and its components"
        ]

        super.init(
            name: "health_check",
            description: "Check the health and status of the Local Intelligence MCP server and its components",
    ...
    private func checkStorageHealth(detailed: Bool) async -> (status: HealthStatus, result: [String: Any], issues: [String]) {
        ...
        if detailed {
            result["storagePaths"] = [
                "logs": "~/.local-intelligence-mcp/logs",
                "config": "~/.local-intelligence-mcp/config",
                "temp": "/tmp/local-intelligence-mcp"
            ]
        }
