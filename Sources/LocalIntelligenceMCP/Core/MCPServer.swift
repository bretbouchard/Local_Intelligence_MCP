...
actor MCPServer {
    ...
    func start() async throws {
        ...
        await logger.info("Starting Local Intelligence MCP", category: .server, metadata: [:])
        ...
        await logger.info("Local Intelligence MCP started successfully", category: .server, metadata: [:])
    }
    ...
    func stop() async {
        ...
        await logger.info("Stopping Local Intelligence MCP", category: .server, metadata: [:])
        ...
        await logger.info("Local Intelligence MCP stopped", category: .server, metadata: [:])
    }
    ...
    private func getMemoryUsage() -> Int64 {
        // Cross-platform memory usage estimation
        #if os(Linux)
        // For Linux, read /proc/self/status
        if let data = try? String(contentsOfFile: "/proc/self/status"),
           let line = data.split(separator: "\n").first(where: { $0.contains("VmRSS:") }) {
            let parts = line.split(separator: " ")
            if parts.count >= 2,
               let kbValue = Int(parts[1]) {
                return Int64(kbValue * 1024) // Convert KB to bytes
            }
        }
        return 0
        #else
        // For macOS and other platforms, use a simple estimate
