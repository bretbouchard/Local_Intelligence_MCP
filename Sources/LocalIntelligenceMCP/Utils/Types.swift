...
/// MCP response content wrapper
struct MCPResponseContent: Sendable {
    let content: AnyCodable
    let metadata: [String: AnyCodable]

    init(content: AnyCodable, metadata: [String: Any] = [:]) {
        self.content = content
        self.metadata = metadata.mapValues { AnyCodable($0) }
    }

    var description: String {
        return String(describing: content.value)
    }
}
...
public struct MCPResponse: Codable, Sendable {
    ...
    public let error: LocalMCPError?
    public let executionTime: TimeInterval?

    public init(success: Bool, data: AnyCodable? = nil, error: LocalMCPError? = nil, executionTime: TimeInterval? = nil) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        error = try container.decodeIfPresent(LocalMCPError.self, forKey: .error)
...
public struct LocalMCPError: Codable, Error {
    ...
    // Static error types
    static let timeout = LocalMCPError(code: "TIMEOUT", message: "Operation timed out")
    static let notFound = LocalMCPError(code: "NOT_FOUND", message: "Resource not found")
    static let invalidParameters = LocalMCPError(code: "INVALID_PARAMETERS", message: "Invalid parameters provided")
    static let permissionDenied = LocalMCPError(code: "PERMISSION_DENIED", message: "Permission denied")
    static let internalError = LocalMCPError(code: "INTERNAL_ERROR", message: "Internal server error")
}
...
/// Policy object for tool execution control
struct ToolExecutionPolicy: Codable, Sendable {
    let allowPCC: Bool
    let piiRedact: Bool
    ...
    init(
        ...
        self.piiRedact = piiRedact
        self.maxOutputTokens = maxOutputTokens
        self.temperature = temperature
        self.stopTokens = stopTokens
    }

    /// Default policy for safe operation
    static let `default` = ToolExecutionPolicy()

    /// Permissive policy for development
    static let permissive = ToolExecutionPolicy(
        allowPCC: true,
        piiRedact: false,
    ...
    static let strict = ToolExecutionPolicy(
        allowPCC: false,
        piiRedact: true,
        maxOutputTokens: 256,
        temperature: 0.1
...
extension Error {
    /// Convert any error to LocalMCPError format
    var mcpError: LocalMCPError {
        if let mcpError = self as? LocalMCPError {
            return mcpError
        }
        ...
        return LocalMCPError(
            ...
            details: ["errorType": AnyCodable(String(describing: type(of: self)))]
        )
    }
}
...
public struct AnyCodable: Codable, @unchecked Sendable {
    ...
    /// Get as text if the underlying value is a string
    var asText: String? {
    ...
    func toAnyDictionary() -> [String: Any] {
        if let dict = value as? [String: Any] {
            ...
            return codableDict.mapValues { $0.value }
        }
        return [:]
    }

    /// Convert AnyCodable dictionary to regular dictionary
    static func toAnyDictionary(_ dict: [String: AnyCodable]) -> [String: Any] {
...
struct MCPConstants {
    ...
    struct Server {
        static let name = "Local Intelligence MCP"
