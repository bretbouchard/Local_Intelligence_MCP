...
extension Error {
    /// Convert any Error to MCP-compliant error format
    var mcpError: MCPError {
        return MCPError(
            code: self.errorCode,
            message: self.localizedDescription,
            details: errorDetails.mapValues { AnyCodable($0) }
        )
    }

    /// Extract error code for MCP protocol
    private var errorCode: String {
        if let mcpError = self as? MCPError {
            return mcpError.code
        }

        if let securityError = self as? SecurityError {
            return "SECURITY_ERROR"
        }

        if let configurationError = self as? ConfigurationError {
            return "CONFIGURATION_ERROR"
        }

        if let keychainError = self as? KeychainError {
    ...
    /// Extract error details for MCP protocol
    private var errorDetails: [String: Any] {
        var details: [String: Any] = [
            "errorType": String(describing: type(of: self))
        ]

        if let nsError = self as NSError? {
            details["nsErrorCode"] = nsError.code
            details["nsErrorDomain"] = nsError.domain
        }
